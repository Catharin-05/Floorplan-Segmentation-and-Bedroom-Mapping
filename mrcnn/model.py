...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    # Convert the dictionary to a list of weights, ensuring all class indices are represented
    num_classes = max(class_weight.keys())  # Find the highest class index
    class_weights_list = [class_weight.get(i, 1.0) for i in range(num_classes + 1)]  # Default to 1.0 if a class is missing

    # Convert the list into a TensorFlow tensor
    class_weight = tf.constant(class_weights_list, dtype=tf.float32)
    weights = tf.gather(class_weight, target_class_ids)
    loss = loss * weights
